 ...
 function addEdificio(lon, lat, nombre, image, id, pisos, v1, v2, v3, v4, tipo) {
     try {
         ...
         var salones = JSON.parse(App.getEdificioSalones(id));
         ...
         console.error("error! while adding " + edificio.get('nombre') + ": ", error)
     }
 }
 ...
 document.addEventListener("DOMContentLoaded", function () { //Listens for typing in search bar
     ...
     if (App.getSalonid() != "") {
         travelToSalon(getSalonFeaturefromID(App.getSalonid()));
     }
 });
 ...
 function handleSearch(searchTerm) {
     ...
     resultBox.style.display = 'block';
     var results = JSON.parse(App.getEdificioSearch(searchTerm));
     results.push(...JSON.parse(App.getSalonSearch(searchTerm)));
 ...
 function travelToEdificio(edificio) {
 ...
 function addRoomsInPolygon(edificio, salones, selectedFloor) {
     ...
     var width = maxX - minX;
     var filteredViewableRooms = viewableRooms.filter(room => {
         var customParams = JSON.parse(App.getCustomParametersSalon(room.salonid));
         return !(customParams && customParams.length > 0);
     });
     ...
     for (var i = 0; i < Object.keys(viewableRooms).length; i++) {
         var customParams = JSON.parse(App.getCustomParametersSalon(viewableRooms[i].salonid));
 ...
 function getSalonFeaturefromID(salonid) {
     var salonFound = undefined;
     JSON.parse(App.getSalon(salonid)).forEach(function (sal) {
